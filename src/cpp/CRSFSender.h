...
#include "CRSFParser.h"  // Include the separate parser header
...
public:
    bool begin(int baud_rate) {
        ...
        for (const char* p : ports) {
            ...
            if (fd >= 0) {
                std::cout << "CRSF: Successfully opened " << p << std::endl;
                found = true;
                break;
            }
        }
        ...
        if (!found) {
            perror("CRSF Error: Could not open /dev/ttyUSB0 or /dev/ttyACM0");
            return false;
        }
    ...
    void send_channels(const int* logical_channels) {
        ...
        std::cout << "CRSF: Sent channels frame (" << written << " bytes)" << std::endl;
    }
};
