...
def draw_controller_icon(screen, x, y, connected):
    ...
    # Simple, Modern Pro Controller Body (No extra lines or circles)
...
def draw_settings_panel(screen, rect, touch_down, touch_x, touch_y, active_index):
    ...
    for i, (name, func) in enumerate(PANEL_MAP):
        ...
        if name == "Back": bcolor = (120, 60, 60) if is_pressed else (80, 40, 40)
        elif i == active_index: bcolor = (40, 100, 200)
        else: bcolor = (100, 100, 110) if is_pressed else (50, 52, 60)
    ...
    if active_index != -1 and PANEL_MAP[active_index][1] is not None:
        ...
        PANEL_MAP[active_index][1](screen, second_rect, touch_down, touch_x, touch_y)
